...
| 2026-07-04 | Phase 9: wired the existing (previously dead) `ClassificationService.override()` into `TicketService.update()` — `PUT /tickets/{id}` now marks `classification.manual_override=true` whenever `category` and/or `priority` change, satisfying TASKS.md Task 2's "allow manual override" requirement via the existing PUT endpoint (no new route added, matching the documented API surface). |
